...
class AudioEditorDialog(QMainWindow):
    ...
    def _create_transport_bar(self) -> QWidget:
        ...
        btn_5s_style = "QPushButton{padding:6px 16px;font-size:12px;}"
        ...
        if Path(ICONS["rewind"]).exists():
            self.btn_back.setIcon(QIcon(ICONS["rewind"]))
            self.btn_back.setIconSize(QSize(18, 18))
        self.btn_back.setStyleSheet(btn_5s_style)
        ...
        if Path(ICONS["forward"]).exists():
            self.btn_fwd.setIcon(QIcon(ICONS["forward"]))
            self.btn_fwd.setIconSize(QSize(18, 18))
            self.btn_fwd.setLayoutDirection(Qt.RightToLeft)
        self.btn_fwd.setStyleSheet(btn_5s_style)
    ...
    def _show_prompt_examples(self):
        ...
        msg = (
            "Examples:\n\n"
            "1) grab track 16 - Example.flac and create a ringtone by cutting between 1:23 to 1:41 into Example_Ringtone.mp3\n\n"
            "2) grab file 11 - You.flac and cut 00:00:00-00:00:35.735 and starting from 00:00:25.000 start to fade out until it ends, export mp3\n\n"
